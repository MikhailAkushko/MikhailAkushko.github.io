...
Before `ApplyPreparedSource`, the target asset has not been changed yet. This is important: many errors intentionally finish the job before target mutation. If the incremental skip check returns `Skipped`, backup capture and `ApplyPreparedSource` do not run.
...
After a successful backup capture, `ApplyPreparedSource` can still fail if a custom apply strategy or target adapter throws an exception. With the current editor and batch persistence paths, that captured snapshot is not guaranteed to be written to disk when the job later returns `Failed`; it may exist only in the in-memory `ImportBackupHistoryAsset` until another save path marks and saves the asset.
