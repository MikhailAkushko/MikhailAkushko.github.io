...
Practical code templates are in [Custom Import Extensions](CustomImportExtensions.md). Safe apply, backup/restore, and incremental state are described in [Backup / Restore / Incremental State](BackupRestoreIncrementalState.md).
...
| --- | --- | --- | --- |
| Source Provider | Reads raw source data and returns `ImportSourceData`. | [`IImportSourceProvider`](CustomImportExtensions.md#custom-iimportsourceprovider) | Provider does not parse entries and does not change the target. Unity text assets usually use `TextAssetImportSourceProvider`. |
| Parser | Turns `sourceData.RawContent` into a list of source entries. | [`IImportParser`](CustomImportExtensions.md#custom-iimportparser), [`IImportParser<TEntry>`](CustomImportExtensions.md#custom-iimportparser) | `parser.EntryType` matches `profile.EntryType`. Result is stored as prepared source inside the pipeline session. |
| Parser Diagnostics | Reads diagnostics from the parser after parse. | [`IImportParserDiagnosticsProvider`](CustomImportExtensions.md#custom-iimportparser) | These diagnostics are added to the shared `ValidationMessages` list, so parser can show warnings/errors next to validation. |
| Target Entries | Reads current entries from the target asset. | [`IImportTargetAdapter.GetTargetEntries`](CustomImportExtensions.md#custom-iimporttargetadapter) | These entries are used for validation context, diff, target fingerprint, and backup. |
...
| Validation | Checks prepared source with target entries and reference lookup. | [`IImportValidator`](CustomImportExtensions.md#custom-validator) | Parser diagnostics and validation diagnostics are combined in `ImportPreview.ValidationMessages`. |
| Diff | Compares source entries and target entries. | [`IImportDiffEngine`](CustomImportExtensions.md#custom-diff-engine), [`IImportDiffEngineWithResult`](CustomImportExtensions.md#custom-diff-engine) | If diff engine throws an exception, pipeline isolates the error as a warning diagnostic and continues preview without changes. |
| Fingerprint | Builds canonical fingerprints for prepared source and current target. | [`IImportFingerprintBuilder`](CustomImportExtensions.md#custom-fingerprint-builder) | Fingerprint is computed by `ImportJobRunner`, not `ImportJobCorePipeline`. It uses prepared source from pipeline and target entries from adapter. |
...
| Apply Strategy Resolution | Selects strategy from override or default profile strategy. | [`IImportApplyStrategy`](CustomImportExtensions.md#custom-apply-strategy), [`IImportProfile.GetDefaultApplyStrategy()`](CustomImportExtensions.md#custom-iimportprofile) | If strategy is not found, job finishes as `Failed`. |
| Incremental Skip Check | Compares current source/target fingerprints and selected apply strategy against `lastApplied*` state. | [`IImportFingerprintBuilder`](CustomImportExtensions.md#custom-fingerprint-builder), `ImportExecutionProfile.enableIncrementalSkip`, `ImportJobDefinition.EnableIncrementalSkip` | If unchanged, returns `Skipped`; backup and target write do not run. |
...
| Current Target Read | Reads target entries before final entries are built. | [`IImportTargetAdapter.GetTargetEntries`](CustomImportExtensions.md#custom-iimporttargetadapter) | Apply strategy receives both source entries and current target entries. |
| Build Final Entries | Builds the final entries list. | [`IImportApplyStrategy.BuildFinalEntries`](CustomImportExtensions.md#custom-apply-strategy) | Replace/merge/update behavior is decided here. Writing to the Unity asset happens at the `Target Write` stage. |
| Target Write | Writes final entries into the target asset. | [`IImportTargetAdapter.SetTargetEntries`](CustomImportExtensions.md#custom-iimporttargetadapter) | This is the actual target asset mutation point. |
| Final Fingerprint | Computes fingerprints after apply. | [`IImportFingerprintBuilder`](CustomImportExtensions.md#custom-fingerprint-builder) | If fingerprints are computed, config updates `lastApplied*` state. If not, state is cleared. |
...
| --- | --- | --- |
| Read data from a custom source | [`IImportSourceProvider`](CustomImportExtensions.md#custom-iimportsourceprovider) | Source provider settings in `ImportSourceConfig`. |
| Parse a custom source shape | [`IImportParser`](CustomImportExtensions.md#custom-iimportparser) | `selectedParserId` or auto parser resolution. |
| Show parser warnings/errors | [`IImportParserDiagnosticsProvider`](CustomImportExtensions.md#custom-iimportparser) | Automatically, when parser implements the interface. |
| Describe a new target shape | [`IImportProfile`](CustomImportExtensions.md#custom-iimportprofile) + [`IImportTargetAdapter`](CustomImportExtensions.md#custom-iimporttargetadapter) | Profile resolution by target asset and `preferredTargetTypeId`. |
| Change target read/write behavior | [`IImportTargetAdapter`](CustomImportExtensions.md#custom-iimporttargetadapter) | Through the selected profile. |
| Change entry comparison keys | `IIdentityStrategy` | Identity strategy resolution for profile/entry type. |
| Check data before apply | [`IImportValidator`](CustomImportExtensions.md#custom-validator) | `selectedValidatorIds`. |
| Attach external reference data | `IImportReferenceLookupProvider` | Global provider registry, used before validation. |
| Change how changes are displayed | [`IImportDiffEngine`](CustomImportExtensions.md#custom-diff-engine) or [`IImportDiffEngineWithResult`](CustomImportExtensions.md#custom-diff-engine) | `selectedDiffEngineId`. |
| Change incremental fingerprints | [`IImportFingerprintBuilder`](CustomImportExtensions.md#custom-fingerprint-builder) | `selectedFingerprintBuilderId`, execution profile fingerprint settings. |
| Change replace/merge behavior | [`IImportApplyStrategy`](CustomImportExtensions.md#custom-apply-strategy) | `selectedApplyStrategyId`, job override, or default profile strategy. |
| Change backup payload | `IImportBackupCodec` | Through the selected profile. |

This table shows where each task plugs into the pipeline; implementation templates live in the linked custom sections.
