...
If the target shape is custom, use a custom [`IImportProfile`](CustomImportExtensions.md#custom-iimportprofile) and [`IImportTargetAdapter`](CustomImportExtensions.md#custom-iimporttargetadapter).
...
For a custom provider, check that it returns a non-empty `ImportSourceData`. See [Custom `IImportSourceProvider`](CustomImportExtensions.md#custom-iimportsourceprovider).
...
- `Parser is not selected and no auto parser could be resolved for this target.` - no auto parser exists and no [custom parser](CustomImportExtensions.md#custom-iimportparser) is selected.
