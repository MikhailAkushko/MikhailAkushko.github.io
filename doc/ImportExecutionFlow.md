...
- Parses source only through the parser from config.
...
- `ImportSourceConfig` does not store provider-specific settings.
- Built-in `TextAssetImportSourceProvider` remains a simple test/default provider.
- HTTP, Google Sheets, auth headers, and other external sources use a separate `IImportSourceProvider` implementation selected through `selectedSourceProviderId`.

`IImportProfile`

- Describes the target execution contract: target id, entry type, target adapter, backup codec, validators, and default apply strategy.
- Auto profile from `[ImportTargetMetadata]` + `[ImportTargetCollection]` is the primary/default path.
...
switch ExecutionMode
     ...
     |
     +-- Apply        -> incremental check -> backup -> apply -> fingerprints -> last applied state
```
...
Resolve parser from config or auto target parser
Build preview through ImportJobCorePipeline
Fill preview stats
Compute canonical source fingerprint
Compute canonical target fingerprint
Return result
```
...
else if target metadata + ImportTargetCollection can create auto parser:
    use auto_json::<targetTypeId>
else:
...
parsed source object = null
```

If parser is not selected, cannot be resolved, and no auto parser can be created for the target, the job fails before preview.
...
Run DefaultDiffEngine
Return ImportPreview
```

Prepared source entries remain inside `ImportJobCorePipeline` for the current `ImportJobRunner`. They are needed for apply and canonical source fingerprint.
...
Compute final canonical source fingerprint
Compute final canonical target fingerprint
