...
For a single-document scenario, use [`[ImportTargetMetadata]`](ImportAttributes.md#importtargetmetadatatargettypeid-displayname-entrytype) on the class and [`[ImportTargetDocument]`](ImportAttributes.md#importtargetdocument) on the field or property where the document is stored:
...
By default, backup capture is enabled even when no `Execution Profile` is assigned. Before the first `Apply`, either assign a `Backup History` asset or assign an `Execution Profile` with `Capture Backup On Apply` disabled. To create a backup history asset, use:

```text
Create > Json Data Importer > Import Backup History
```

The full field list and policy override order are described in [ImportSourceConfig Reference](ImportSourceConfigReference.md).
...
- `Apply` - run the preview path again, capture a backup when backup capture is enabled, then apply prepared source to the target asset. If backup capture is enabled and no `Backup History` asset is assigned, `Apply` stops before changing the target.
...
A custom parser is needed when source data cannot be parsed directly by the auto JSON or CSV parser into your entry type.
...
- Source is not JSON/CSV at all or comes in a custom format.

The following custom examples use this minimal target shape:
...
[CreateAssetMenu(
    fileName = "HardJsonStationDatabase",
    menuName = "Game/Import/Hard JSON Station Database")]
public sealed class HardJsonStationDatabase : ScriptableObject
{
...
After compilation, the parser appears in the Parser dropdown when its `entryType` matches the entry type of the selected target/profile. In the config, select this parser instead of the auto JSON parser. For the full parser template and requirements, see [Custom `IImportParser`](CustomImportExtensions.md#custom-iimportparser).
...
- there is no [`[ImportTargetCollection]`](ImportAttributes.md#importtargetcollection) or [`[ImportTargetDocument]`](ImportAttributes.md#importtargetdocument);
...
Simplified adapter/profile shape:
...
public sealed class HardJsonStationTargetAdapter : IImportTargetAdapter
{
    ...
    private static SpaceStation CloneStation(object sourceEntry)
    {
        ...
        // Copy nested objects and collections here as new instances too.
...
After compilation, the profile is found by the discovery system if it is a concrete `IImportProfile` with a public parameterless constructor. In `ImportSourceConfig`, select this profile in the import profile field. For complete adapter/profile templates, see [Custom `IImportTargetAdapter`](CustomImportExtensions.md#custom-iimporttargetadapter) and [Custom `IImportProfile`](CustomImportExtensions.md#custom-iimportprofile).
...
- built-in `Text Asset`, `File Path`, or `HTTP` providers do not cover the source;
- the source needs Google Sheets access, custom authentication, generated content, decrypted content, or another project-specific fetch step.

Execution path details are described in [Import Execution Flow](ImportExecutionFlow.md). Custom source providers are covered in [Custom `IImportSourceProvider`](CustomImportExtensions.md#custom-iimportsourceprovider).
